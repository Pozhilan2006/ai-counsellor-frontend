...
export default function Navbar() {
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 w-full z-50 transition-colors duration-500 ${glassy
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <Link
            href="/login"
            className={`text-sm font-medium hover:opacity-80 transition-all duration-300 ${glassy ? "text-slate-600" : "text-white/80 hover:text-white"
              }`}
          >
            Log in
          </Link>
          <Link
            href="/onboarding"
            className={`px-5 py-2.5 text-sm font-medium rounded-full transition-all duration-300 shadow-lg hover:shadow-xl hover:-translate-y-0.5 ${glassy

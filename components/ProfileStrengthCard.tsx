...
export function ProfileStrengthView({ data, isLoading, error }: { data: ProfileStrengthData | null, isLoading: boolean, error: string | null }) {
    ...
    return (
        <div className="card-glass p-6 relative overflow-hidden h-full">
            ...
            <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden mb-8">
                <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${data.completion_score}%` }}
            ...
            <div className="space-y-4">
                {[
                    { label: "Academics", data: data.academics },
                    { label: "Exams", data: data.exams },
                    { label: "SOP", data: data.sop },
                    { label: "Documents", data: data.documents },
                ].map((item) => (
...
export default function ProfileStrengthCard({ email }: ProfileStrengthCardProps) {
    ...
    return <ProfileStrengthView data={strengthData} isLoading={isLoading} error={error} />;
}
